...
class Pregunta {
  constructor(enunciado, A, B, C, D, correcta, identificador) {
...
function leerPreguntas() {
  let memoriaArray = JSON.parse(localStorage.getItem("preguntasV2"));
  ...
  switch (evento.target.id) {
    ...
    case "respuestaD":
      ...
      if (evento.target.validity.valueMissing) {
        ...
        formulario[evento.target.id].control.placeholder =
          formulario[evento.target.id].vacio;
      } else {
...
function comprobarRadio() {
  if (formulario["correcta"].control.value != "") {
    ...
    errores.innerHTML =
      '<p style="color:red">Marque una respuesta correcta</p>';
  }
}
...
function recomprobar() {
  ...
  if (controles.every((control) => control.valido === true)) {
    ...
    let resCorrecta =
      formulario[`respuesta${formulario.correcta.control.value}`].control.value;
    ...
    localStorage.setItem("preguntasV2", JSON.stringify(arrayPreguntas));
  }
}
...
function agregarPregunta() {
  ...
  if (document.forms[1]) {
    document.forms[1].parentElement.replaceChild(
      formularioNuevo,
      document.forms[1]
    );
  } else {
...
function corregirExamen() {
  ...
  for (element of fieldsetPreguntas) {
    ...
    let preguntaSeleccionada = arrayPreguntas.find(
      (dato) =>
        dato.enunciado === element.getElementsByTagName("p")[0].innerText
    );
