...
<body>
    ...
    function realizarDesplegable($conexion, $cadena)
    {
        ...
        if ($resultado != null) {
            ...
            do {
                ...
                if($row!=null)
                $cadena .= '<option value="' . $row[0] . '">' . $row[1] . '</option><br>';
    ...
    function mostrarStock($conexion, $cadena)
    {
        ...
        foreach ($_POST['tienda'] as $selected) {
            ...
            $sql = 'SELECT producto.nombre_corto, stock.unidades FROM producto, stock WHERE (producto.cod=stock.producto) AND (stock.tienda="' . $selected . '")';
        ...
        $cadena .= "</table>";
        $cadena .= '<button onclick="window.location.href="SeleccionarTienda.php">Ver tiendas</button>';
        return $cadena;
    }
