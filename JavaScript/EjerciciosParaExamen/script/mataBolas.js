btnAceptar.onclick = guardarNombre;
...
function creacionBolas() {
    ...
    }, (Math.random() * 30) + 5)//el tiempo de intervalo tambien sera random dando un +5 por si el numero es muy bajo que podamos pararlo
...
function mostrarRanking() {
    ...
    if (ranking != null) {
        candenaTexto = "<table><tr><th>Posicion</th><th>Usuario</th><th>Puntuacion</th></tr>";
        ranking.forEach(function (element,indice ){
            candenaTexto += `<tr><td>${indice+1}</td><td>${element.nombre}</td><td>${element.puntos}</td></tr>`;            
        });
        candenaTexto += "</table>";
