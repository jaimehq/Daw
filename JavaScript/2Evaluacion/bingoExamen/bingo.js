...
function seleccionCartones() {
  ...
  fieldsetNuevo.firstElementChild.innerText =
    "Elige el numero de cartones que deseas jugar:";
...
function pintarSacado(numero) {
  ...
  let celdaSeleccionada = arrayCeldas.find(
    (celda) => celda.innerText == numero
  );
  celdaSeleccionada.setAttribute("class", "sacado");
}
function pintarPvp(numero) {
  let tabla = document.getElementById("cartonPvp");
  let celdas = tabla.getElementsByTagName("td");
  let arrayCeldas = Object.values(celdas);
  let celdaSeleccionada = arrayCeldas.find(
    (celda) => celda.innerText == numero
  );
  celdaSeleccionada.setAttribute("class", "sacadoPvp");
}
function generarCarton(arrayNumeros) {
  ...
  let z = 0;
  arrayNumerosCarton.sort(function (a, b) {
    return a - b;
  });
  for (let i = 0; i < 3; i++) {
    let fila = document.createElement("tr");
...
function perderLinea() {
  if (!lineaCantada) {
    let cartonPvp = document.getElementById("cartonPvp");
    let lineas = cartonPvp.children;
    let arrayLineas = Object.values(lineas);
    arrayLineas.forEach((linea) => {
      let celdas = linea.children;
      let valores = [];
      for (celda of celdas) {
        valores.push(celda.className);
      }

      celdasBool = valores.every((celda) => {
        return celda == "sacadoPvp";
      });
      if (celdasBool) {
        lineaBtn.setAttribute("disabled", true);
        let textoLinea = document.createElement("p");
        textoLinea.innerText =
          "Ya se ha cantado linea y la has perdido paquete";
...
function perderBingo() {
  let correcta = false;
  let carton = document.getElementById("cartonPvp");
  ...
  for (linea of arrayLineas) {
    let celdas = linea.children;
    for (celda of celdas) {
      valoresCarton.push(celda.className);
    }
  ...
  if (correcta == true) {
    let contenido = [...document.body.children];
    contenido.forEach((elemento) => {
      elemento.remove();
    });
    let textoLinea = document.createElement("h1");
    textoLinea.innerText = "Has palmado paquete";
    document.body.appendChild(textoLinea);
  }
}
...
function ganarBingo() {
  ...
  let plantillaFormulario =
    document.getElementById("formularioGanador").content;
